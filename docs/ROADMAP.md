...
- [x] **First-class agent integration** — a hook installer (`--install-hook` /
  `--uninstall-hook`, marker-based clean uninstall for Claude Code + Codex), an
...
- [x] **Stale-holder CPU evidence** — sample each assertion holder's process-tree
  CPU and label a hold from a ~0%-CPU process as likely stale ("held 2 h · ~0%
  CPU"), upgrading the sleep verdict from classification to evidence. Shipped in
  1.18.0 (single-enumeration multi-PID sampling with a 10-minute stale window).
